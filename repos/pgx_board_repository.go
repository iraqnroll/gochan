...
const (
	get_all_query           = `SELECT id, uri, name, description FROM boards`
	get_by_id_query         = `SELECT uri, name, description FROM boards where id = $1`
	get_by_uri_query        = `SELECT id, name, description FROM boards where uri = $1`
	get_all_for_admin_query = `SELECT 
		b.id,
		b.uri,
		b.name,
		b.description,
	...
	create_new_query = `INSERT INTO boards (uri, name, description, ownerId)
		VALUES ($1,$2,$3,$4) RETURNING id, to_char(date_created, 'YYYY-MM-DD HH24:MI:SS')`
	delete_query = `DELETE FROM boards WHERE id = $1`
)
...
func (r *PostgresBoardRepository) Delete(id int) error {
	_, err := r.db.Exec(delete_query, id)
	if err != nil {
...
func (r *PostgresBoardRepository) CreateNew(uri, name, description string, owner_id int) (models.Board, error) {
	...
	row := r.db.QueryRow(create_new_query, result.Uri, result.Name, result.Description, result.OwnerId)
...
func (r *PostgresBoardRepository) GetAllForAdmin() ([]models.Board, error) {
	...
	rows, err := r.db.Query(get_all_for_admin_query)
	if err != nil {
...
func (r *PostgresBoardRepository) GetByUri(uri string) (models.BoardDto, error) {
	...
	rows := r.db.QueryRow(get_by_uri_query, result.Uri)
...
func (r *PostgresBoardRepository) GetById(id int) (models.BoardDto, error) {
	...
	rows := r.db.QueryRow(get_by_id_query, result.Id)
